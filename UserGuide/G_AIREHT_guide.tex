...
This document provides the steps for running G\_AIREHT and how to quickly processes the output on the Compute Canada network of supercomputers. Here are some useful links for getting started with Compute Canada:
\begin{itemize}
  ...
  %\item[--] \href
\end{itemize}
G\_AIREHT uses Python3. If you are new to python, a good place to get started is with \href{https://pyzo.org/}{Pyzo}. Some other helpful links for getting started with Python:
\begin{itemize}
...
G\_AIREHT requires several input files formatted in a specific way to run. The program will prompt you for these files as it runs. Examples of these files can be found in the TestData folder.
\begin{enumerate}
  ...
  \item \textbf{Blob Data} The program requires the file outputted by \href{https://www.ctlab.geo.utexas.edu/software/blob3d/}{Blob3D} after processing CT data to isolate garnets in a rock. The file will need to be converted to a xlsx file to be read by G\_AIREHT.
\end{enumerate}
...
\end{gather*}
Where $z$ is the number of cations in oxide $i$, $m$ is the mass of the oxide, $M$ is the molar mass of the oxide. Note that Fe3+ and Fe2+ will both be added together as Fe2+. Oxygen is recalculated assuming reduced oxides. H$_2$O is assumed to be in excess with 200mol of H and 100mol of O CO$_2$ is assumed to be reduced and in excess with 100 mol of C.
...
\subsection{Growing garnet}
Once the inital conditions are set, the garnet needs to start ``growing''. Growth occurs at a constant radial rate with one layer added at a time with a thickness of $dr$. Every garnet that has been nucleated by the program will have a layer added with the same composition. The steps taken in the algorithm are outlined below:
\begin{enumerate}
\item Nucleate the first garnet with radius $r = dr$ with the composition of the first segment
\item Add a layer of thickness $dr$ to the garnet with the composition of the next segment
...
The total mols of a component in all garnet can then be removed from the bulk rock composition to determine the amount of material fractionated from the system. The program does this calculation at every radius interval defined by the user for creating the output files.
\end{document}
